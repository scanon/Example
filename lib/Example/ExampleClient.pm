...
sub new
{
    ...
    my $self = {
	client => Example::ExampleClient::RpcClient->new,
	url => $url,
	headers => [],
    };
...
<pre>
$params is an Example.FilterContigsParams
$output is an Example.FilterContigsResults
FilterContigsParams is a reference to a hash where the following keys are defined:
	assembly_input_ref has a value which is an Example.assembly_ref
...
FilterContigsResults is a reference to a hash where the following keys are defined:
	...
	assembly_output has a value which is an Example.assembly_ref
...
sub _validate_version {
    ...
    if ($sMinor > $cMinor) {
        warn "New client version available for Example::ExampleClient\n";
    }
    if ($sMajor == 0) {
        warn "Example::ExampleClient version is $svr_version. API subject to change.\n";
    }
}
...
package Example::ExampleClient::RpcClient;
